...
namespace TotalUpdater.Next.Core.Installation
{
    ...
    {
        public static WcxRegistrationHarvestFinding FromInspectedPackage(PluginCatalogEntry entry, PackageInspection package, WcxProbeResult x86, WcxProbeResult x64, string source)
        {
            ...
            var x86Files = package.Files.Where(x => x.RelativePath.EndsWith(".wcx", StringComparison.OrdinalIgnoreCase) || x.RelativePath.EndsWith(".uwcx", StringComparison.OrdinalIgnoreCase)).ToList();
            var x64Files = package.Files.Where(x => x.RelativePath.EndsWith(".wcx64", StringComparison.OrdinalIgnoreCase)).ToList();
            // Selecting an arbitrary binary would turn a harvest finding into
            // untrustworthy installation evidence.  A package must identify one
